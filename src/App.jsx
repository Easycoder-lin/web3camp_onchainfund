...
import { id } from "ethers"; // built-in keccak256 of signature
...
export default function App() {
    ...
    const handleChainChanged = async (hexId) => {
      ...
      setStatus(
        id !== REQUIRED_CHAIN_ID
          ? {
              type: "error",
              message: "Wrong network. Please switch to Sepolia (11155111).",
            }
  ...
  async function connect() {
    try {
      ...
      if (parseInt(cidHex, 16) !== REQUIRED_CHAIN_ID) {
        ...
        if (parseInt(cidHex, 16) !== REQUIRED_CHAIN_ID) {
          throw new Error(
            "Please switch MetaMask to Sepolia (chainId 11155111)."
          );
        }
      }
  ...
  async function transfer() {
    ...
    try {
      ...
      if (mode === "NATIVE") {
        ...
        setStatus({
          type: "success",
          message: `Native transfer confirmed in block ${rec.blockNumber}.`,
          hash: tx.hash,
        });
      } else {
        ...
        setStatus({
          type: "success",
          message: `${amount} ${tokenSymbol || "tokens"} transferred. Block ${
            rec.blockNumber
          }.`,
          hash: tx.hash,
        });
      ...
      if (provider && account)
        await refreshBalances(provider, account, tokenAddr);
    } catch (err) {
      console.error(err);
      setStatus({ type: "error", message: shortErr(err) });
    }
  ...
  async function handleApproveAndBuyShares() {
    try {
      ...
      if (!ethers.isAddress(cpAddr))
        throw new Error("Invalid ComptrollerProxy");
      ...
      setStatus({
        type: "success",
        message: `Subscribed. Block ${rc.blockNumber}.`,
        hash: tx2.hash,
      });
    } catch (err) {
      console.error(err);
  ...
  async function handleBuySharesWithEth() {
    try {
      ...
      if (!ethers.isAddress(cpAddr))
        throw new Error("Invalid ComptrollerProxy");
      if (!investAmt || Number(investAmt) <= 0)
      ...
      try {
        ...
        if (
          ENZYME?.WETH &&
          ethers.isAddress(ENZYME.WETH) &&
          denom?.toLowerCase?.() !== ENZYME.WETH.toLowerCase()
        ) {
          throw new Error(
            "buySharesWithEth is only supported when denomination asset is WETH"
          );
        }
      } catch {
        /* 沒 getter 就放行，由合約自行判斷 */
      }
      ...
      setStatus({
        type: "success",
        message: `Subscribed with ETH. Block ${rc.blockNumber}.`,
        hash: tx.hash,
      });
    } catch (err) {
      console.error(err);
  ...
  async function wrapEthQuick() {
    try {
      ...
      if (!ethers.isAddress(wethAddr))
        throw new Error("Set denomination token (WETH) first");
      ...
      setStatus({
        type: "success",
        message: `Wrapped to WETH. Block ${rc.blockNumber}.`,
        hash: tx.hash,
      });
    } catch (err) {
      console.error(err);
  ...
  async function handleRedeemInKind() {
    try {
      ...
      setStatus({
        type: "success",
        message: `Redeemed (in-kind). Block ${rc.blockNumber}.`,
        hash: tx.hash,
      });
      await fetchMyShares?.();
    } catch (err) {
  ...
  async function handleRedeemSpecific() {
    try {
      ...
      if (assets.length !== percents.length)
        throw new Error(
          `Assets (${assets.length}) and percentages (${percents.length}) mismatch`
        );

      const tx = await cp.redeemSharesForSpecificAssets(
        account,
        sharesQty,
        assets,
        percents
      ...
      setStatus({
        type: "info",
        message: `Redeeming for specific assets…`,
        hash: tx.hash,
      });
      const rc = await tx.wait();
      setStatus({
        type: "success",
        message: `Redeemed (specific). Block ${rc.blockNumber}.`,
        hash: tx.hash,
      });
      await fetchMyShares?.();
    } catch (err) {
  ...
  async function checkVaultHasSellToken(
    cpAddrUse,
    sellToken,
    needAmountBN,
    sellTokenDecimals
    ...
    if (bal < needAmountBN) {
      ...
      throw new Error(
        `Vault ${shortAddr(vault)} 的 ${
          sym || "token"
        } 餘額不足：擁有 ${have}，需要 ${need}`
      );
    }
    return { vault, sym };
  }
...
async function handleSwapViaIM() {
  try {
    ...
    // 4) Read decimals + amounts
    const ercSell = new ethers.Contract(sellToken, ERC20_ABI, provider || signer);
    const ercBuy = new ethers.Contract(buyToken, ERC20_ABI, provider || signer);

    const sellDec = await ercSell.decimals().catch(() => 18);
    const buyDec = await ercBuy.decimals().catch(() => 18);

    if (sellDec !== buyDec) {
      console.warn(
        `⚠️ Token decimals mismatch: sellToken(${sellDec}) vs buyToken(${buyDec}).`
      );
    }

    const amountIn = ethers.parseUnits(String(sellAmt ?? "0"), Number(sellDec));
    const minAmount = ethers.parseUnits(String(minBuyAmt ?? "0"), Number(buyDec));
    if (amountIn <= 0n) throw new Error("Sell amount must be > 0");

    await checkVaultHasSellToken(cpAddrUse, sellToken, amountIn, sellDec);

    // 5) Encode integrationData (adapter expects [address[], uint256, uint256])
    const coder = ethers.AbiCoder.defaultAbiCoder();
    const integrationData = coder.encode(
      ["address[]", "uint256", "uint256"],
      [path, amountIn, minAmount]
    );

    // 6) Selector for takeOrder(address,bytes,bytes)
    const iface = new ethers.Interface([
      "function takeOrder(address,bytes,bytes)"
    ]);
    const sel = iface.getFunction("takeOrder").selector;
    console.log("Selector:", sel); // should log 0xec4b2ef6

    // 7) Pack for IM: abi.encode(adapter, selector, integrationData)
    const callArgs = coder.encode(
      ["address", "bytes4", "bytes"],
      [ADP, sel, integrationData]
    );

    // 8) Send via ComptrollerProxy
    const cp = new ethers.Contract(
      cpAddrUse,
      ["function callOnExtension(address,uint256,bytes)"],
      signer
    );

    // Pre-flight
    try {
      await cp.callOnExtension.staticCall(IM, INTEGRATION_ACTION_CALL ?? 0, callArgs);
    } catch (e) {
      throw new Error(
        `Simulation reverted: ${shortErr(e)}\n` +
        "Possible issues:\n" +
        "① Vault doesn’t hold enough sellToken\n" +
        "② No UniswapV2 pool/liquidity for this path\n" +
        "③ Adapter not whitelisted for fund\n" +
        "④ Policy blocking swap\n" +
        "⑤ minAmountOut too high (try 0)"
      );
    }

    // Gas estimate
    const gas = await cp.callOnExtension.estimateGas(
      IM,
      INTEGRATION_ACTION_CALL ?? 0,
      callArgs
    );

    const tx = await cp.callOnExtension(
      IM,
      INTEGRATION_ACTION_CALL ?? 0,
      callArgs,
    ...
    setStatus({
      type: "info",
      message: "Swap via IM (Uniswap V2)…",
      hash: tx.hash,
    });
    ...
    setStatus({
      type: "success",
      message: `Swap confirmed. Block ${rc.blockNumber}.`,
      hash: tx.hash,
    });
    ...
    setStatus({
      ...
      message:
        shortErr(err) ||
        "Swap reverted. Check vault balances, path, adapter, policies, or minAmountOut.",
    });
  }
}
  ...
  async function detectDenominationAddress(e) {
    try {
      ...
      if (!ethers.isAddress(addr))
        throw new Error("Getter returned non-address");
      ...
      setStatus({
        type: "success",
        message: `Detected denomination asset: ${ethers.getAddress(addr)}`,
      });
    } catch (err) {
      console.error(err);
      setStatus({
        ...
        message:
          shortErr(err) ||
          "Cannot detect denomination asset on this comptroller. Please paste the token address manually.",
      });
    }
  }
  ...
  async function resolveDenominationAddressOrThrow() {
    ...
    throw new Error(
      "Denomination asset unknown. Paste the token address in the Invest panel."
    );
  }
  ...
  async function estimateMinShares(bps = 100) {
    ...
    if (!investAmt || Number(investAmt) <= 0)
      throw new Error("Enter investment amount first");
    ...
    if (gsv === 0n) {
      ...
      setStatus({
        type: "info",
        message: "Share value is 0; set min shares = 0 for first subscription.",
      });
      return;
    }
    ...
    setStatus({
      type: "success",
      message: `Estimated min shares (~${(100 - bps / 100).toFixed(
        2
      )}% of expected)`,
    });
  }
  ...
  async function ensureAllowance(erc, owner, spender, amt, label = "spender") {
    ...
    try {
      ...
      setStatus({
        type: "info",
        message: `Reset allowance to 0 for ${label}…`,
        hash: tx0.hash,
      });
      await tx0.wait();
          ...
          <div className="p-4 rounded-2xl bg-amber-50 text-amber-900 border border-amber-200 flex gap-3 items-start">
            ...
            <div>
              ...
              <p className="text-sm opacity-80">
                Install the extension and refresh this page.
              </p>
            </div>
          </div>
        )}
        ...
        >
          ...
          {appTab === "INVEST" && (
            <div className="bg-white p-6 rounded-2xl shadow-lg border grid gap-4">
              ...
              <div className="grid gap-2">
                ...
                <div className="grid gap-2">
                  ...
                  {denomSymbol && (
                    <p className="text-xs text-gray-500">
                      Detected token:{" "}
                      <span className="font-medium">{denomSymbol}</span>{" "}
                ...
                <div className="grid md:grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      ...
                      value={investAmt}
                      onChange={(e) =>
                        setInvestAmt(e.target.value.replace(/[^0-9.]/g, ""))
                      }
                      placeholder="e.g. 1.0"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      value={minShares}
                      onChange={(e) =>
                        setMinShares(e.target.value.replace(/[^0-9.]/g, ""))
                      }
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  ...
                  {denomAddr &&
                    ENZYME.WETH &&
                    denomAddr.toLowerCase() === ENZYME.WETH.toLowerCase() && (
                      <>
                        <button
                ...
                <p className="text-xs text-gray-500">
                  順序：先 approve（Vault / Comptroller），再 buyShares（或用
                  ETH 直購）。
                </p>
                ...
                <div className="pt-2">
                  ...
                  {myShares && (
                    <div className="mt-2 text-sm text-gray-600">
                      ...
                      <div>
                        Your shares:{" "}
                        <span className="font-semibold">{myShares.amount}</span>{" "}
                        {myShares.symbol}
                        {myShares.total && (
                          <>
                            {" "}
                            • Total supply:{" "}
                            <span className="font-mono">{myShares.total}</span>
                          </>
                        )}
                      </div>
                    </div>
              ...
              <div className="grid gap-2">
                <label className="text-sm font-medium">
                  VaultProxy（可留空自動從 Comptroller 取得）
                </label>
                <input
                  className="w-full px-4 py-2 border rounded-lg"
                  value={vaultAddr}
                ...
                <input
                  ...
                  value={redeemShares}
                  onChange={(e) =>
                    setRedeemShares(e.target.value.replace(/[^0-9.]/g, ""))
                  }
                  placeholder="e.g. 10.0"
                />
                <div className="flex gap-2">
                  <button
                    onClick={handleRedeemInKind}
                ...
                <div className="grid md:grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="0x...,0x... 或 WETH,USDC"
                    />
                  </div>
                  <div>
                    <label className="text-sm">
                      Percentages (sum=10000 or 70,30 / 0.7,0.3)
                    </label>
                    <input
                      className="w-full px-4 py-2 border rounded-lg"
                      value={redeemPercents}
                      onChange={(e) => setRedeemPercents(e.target.value)}
                      placeholder="7000,3000 / 70,30 / 0.7,0.3"
                    />
                  </div>
                </div>
                <p className="text-xs text-gray-500">
                  In-Kind 依持倉比例；Specific
                  可指定資產與比例（可填小數或百分比，會自動換算成 10000 bps）。
                </p>
              </div>
            </div>
          )}
          ...
          {appTab === "SWAP" && (
            <div className="bg-white p-6 rounded-2xl shadow-lg border grid gap-3">
              <h2 className="text-lg font-semibold">
                Swap via IntegrationManager (Uniswap V2)
              </h2>
              ...
              <div className="grid md:grid-cols-3 gap-2">
                <div>
                  ...
                  <input
                    ...
                    value={sellAmt}
                    onChange={(e) =>
                      setSellAmt(e.target.value.replace(/[^0-9.]/g, ""))
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    value={minBuyAmt}
                    onChange={(e) =>
                      setMinBuyAmt(e.target.value.replace(/[^0-9.]/g, ""))
                    }
                    placeholder="e.g. 20"
                  />
                </div>
              ...
              <p className="text-xs text-gray-500">
                V2 使用{" "}
                <code>orderData = abi.encode(outgoing, minIncoming, path)</code>
                ， selector 固定為 <code>takeOrder(bytes)</code>，path 僅為{" "}
                <code>address[]</code>（不可帶 fee）。
              </p>
            </div>
          )}
          ...
          {appTab === "ENZYME" && (
            <div className="bg-white p-6 rounded-2xl shadow-lg border space-y-4">
                ...
                {chainId && chainId !== REQUIRED_CHAIN_ID && (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="text-sm rounded-lg bg-gray-900 text-white px-3 py-1.5 hover:opacity-90"
                    >
                      Switch
                    </button>
                  </div>
              ...
              <div className="grid gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Fee recipient (for entrance fee)
                  </label>
                  <input
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-300 focus:border-emerald-300 outline-none"
                    placeholder="0x..."
                ...
                <div className="grid gap-2">
                  ...
                  <p className="text-xs text-gray-500">
                    You can either create a new whitelist or paste an existing{" "}
                    <code>listId</code>.
                  </p>
                </div>
          ...
          {status && (
            ...
            >
              ...
              <div>
                ...
                {status.hash && (
                  <p className="text-sm mt-1 opacity-75 break-all">
                    Tx hash: {status.hash}
                  </p>
                )}
              </div>
  ...
  async function handleCreateWhitelist() {
    try {
      ...
      try {
        ...
        for (const log of receipt.logs) {
          if (
            ...
            ENZYME.ADDRESS_LIST_REGISTRY.toLowerCase()
          ) {
            const parsed = iface.parseLog(log);
            if (parsed?.name === "ListCreated") {
              newId =
                parsed.args?.listId?.toString?.() ||
      ...
      if (newId) {
        ...
        setStatus({
          type: "success",
          message: `Whitelist created. listId=${newId}`,
          hash: tx.hash,
        });
      } else {
        setStatus({
  ...
  async function handleCreateFund() {
    try {
      ...
      for (const [k, v] of Object.entries(ENZYME)) {
        if (!isHexAddress(v))
          throw new Error(`ENZYME.${k} is not a valid address`);
      }
      const listIdNum = listId
        ? (() => {
            try {
              return BigInt(listId);
            } catch {
              return null;
            }
          })()
      ...
      const factory = new ethers.Contract(
        ENZYME.FUND_DEPLOYER,
        FUND_FACTORY_ABI,
        signer
      );

      const feeSettings = coder.encode(
        ["uint256", "address"],
        [100n, feeRecipient]
      ); // 1%
      const fee_full = coder.encode(
        ["address[]", "bytes[]"],
        [[ENZYME.ENTRANCE_RATE_DIRECT_FEE], [feeSettings]]
      );
      ...
      try {
        ...
        try {
          ...
          setStatus({
            type: "info",
            message:
              "Fee/policy likely not accepted by this release. Created without fee/policy.",
          });
        } catch (eB) {
          try {
            result = await tryVariant(
            ...
            setStatus({
              type: "info",
              message:
                "WETH likely not allowed as denomination. Created with USDC, no fee/policy.",
            });
          } catch (eC) {
            throw new Error(
              shortErr(eA) ||
                shortErr(eB) ||
                shortErr(eC) ||
                "Fund creation reverted during gas estimation."
            );
          }
        }
      }
      ...
      if (comp && vault) {
        ...
        setStatus({
          type: "success",
          message: `Fund created. Comptroller: ${shortAddr(
            comp
        ...
        setStatus({
          type: "success",
          message:
            "Fund created. Couldn’t read return values; check tx on explorer.",
        });
      }
    } catch (err) {
      console.error(err);
...
function toAddressLoose(s) {
  ...
  if (map[U]) {
    try {
      return ethers.getAddress(map[U]);
    } catch {}
  }
  // 允許任意大小寫地址：轉小寫再 checksum
  try {
    if (ethers.isAddress(t)) return ethers.getAddress(t);
  } catch {}
